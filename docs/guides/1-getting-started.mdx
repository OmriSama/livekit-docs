...
Alternatively, you can build LiveKit on your machine. To do so, you must have
Go 1.16+ installed and available in your `$PATH`. These steps make use of [Go's
Module system][gomods-explainer].
...
1. Clone the `livekit-server` repository to your machine and navigate into
   it:
...
2. Install [Mage][mage] by cloning it outside of the `livekit-server` repo:
...
4. And finally, install all the necessary modules for the project:

  ```shell
  go mod download
  ```

[gomods-explainer]: https://go.dev/blog/using-go-modules
...
```shell
LIVEKIT_KEYS="<api-key>: <api-secret>" ./bin/livekit-server --dev
```
...
```shell
LIVEKIT_KEYS="<api-key>: <api-secret>" ./bin/livekit-server create-join-token --room myroom --identity $participant_identity
```
...
This token expires after a month, which is useful for development and testing, but inappropriate for use in production. To generate production tokens, see [Access Token guide](/guides/access-tokens).
